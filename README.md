...
- `background.js` calls `https://speedy-gpt.jmw.nz/api/v1/chat/completions` with `stream: true`, parses the OpenAI-style SSE, and posts each `{type:"chunk",delta}` back through the port. The service worker is not bound by page CSP and has `host_permissions` for speedy-gpt.jmw.nz. The `model` field is a placeholder — the proxy's `MODEL` env var is the source of truth and rewrites every request server-side.
